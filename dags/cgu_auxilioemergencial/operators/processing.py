...
from cgu_auxilioemergencial.operators.file_storage import MINIO_BUCKET, MINIO_CONN_ID, FILEPATH_KEY
from cgu_auxilioemergencial.operators.database import INDEX_KEY
...
def _load_people(filepath: str) -> Iterator[Dict[str, Any]]:
    ...
    for i, line in enumerate(reader):
        person = {
            ...
            'ibge_county_code': line['ibge_county_code'].upper(),
            'county': line['county'],
            'nis': line['nis'],
            'cpf': re.sub(r'[^\d\*]', '', line['cpf']),
            'name': line['name'].upper(),
            'responsible': {
                'nis': line['responsible_nis'].upper() if line['responsible_cpf'] else '',
                'cpf': re.sub(r'[^\d\*]', '', line['responsible_cpf']) if line['responsible_cpf'] else '',
                'name': line['responsible_name'].upper() if line['responsible_cpf'] else '',
            },
